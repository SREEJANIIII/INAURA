...
// --- MAIN COMPONENT ---
/** What a real sign-up returns: an error to show, or a success message */
export type SignUpResult = { error: string | null; message?: string };
export type SocialProvider = "google" | "github";

interface AuthComponentProps {
  logo?: React.ReactNode;
  brandName?: string;
  /** Heading on the first step (default "Get started with Us") */
  title?: string;
  /** Creates the account. Without it the form only plays its demo animation. */
  onSignUp?: (email: string, password: string) => Promise<SignUpResult>;
  /** Starts Google/GitHub sign-in. Without it those buttons do nothing. */
  onSocial?: (provider: SocialProvider) => Promise<{ error: string | null }>;
  /** Shown under the form, e.g. a "Log in" link */
  footer?: ReactNode;
  /** "login": email → password → sign in (no confirm step, no confetti) */
  mode?: "signup" | "login";
  /** Signs in (login mode) */
  onLogIn?: (email: string, password: string) => Promise<SignUpResult>;
  /** Sends a password-reset email (login mode shows a "Forgot password?" link) */
  onForgotPassword?: (email: string) => Promise<SignUpResult>;
}

const loginSteps = [
    { message: "Signing you in...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: "Loading your profile...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
    { message: "Almost there...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
];

export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize", title = "Get started with Us", onSignUp, onSocial, footer, mode = "signup", onLogIn, onForgotPassword }: AuthComponentProps) => {
  ...
  const [modalStatus, setModalStatus] = useState<'closed' | 'loading' | 'error' | 'success' | 'info'>('closed');
  const [infoMessage, setInfoMessage] = useState('');
  const [modalErrorMessage, setModalErrorMessage] = useState('');
  const [successMessage, setSuccessMessage] = useState(modalSteps[modalSteps.length - 1].message);
  const confettiRef = useRef<ConfettiRef>(null);

  const isEmailValid = /\S+@\S+\.\S+/.test(email);
  // Signing in accepts any password; creating one needs at least 6 characters
  const isPasswordValid = isLogin ? password.length > 0 : password.length >= 6;
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
        if (authStep !== 'password' || !isPasswordValid || !onLogIn) return;
        ...
        try {
            ...
            setModalErrorMessage("Network error. Please check your connection and try again.");
            setModalStatus('error');
        }
        return;
    }
    ...
    if (password !== confirmPassword) {
        ...
        // Real sign-up: the loading steps play while the account is created
        setModalStatus('loading');
        try {
  ...
  const handleForgot = async () => {
    if (!onForgotPassword || modalStatus !== 'closed') return;
    const result = await onForgotPassword(email.trim());
    if (result.error) {
        setModalErrorMessage(result.error);
        setModalStatus('error');
    } else {
        setInfoMessage(result.message ?? "Password reset email sent. Check your inbox.");
        setModalStatus('info');
    }
  };

  const handleSocial = async (provider: SocialProvider) => {
    if (!onSocial || modalStatus !== 'closed') return;
    const { error } = await onSocial(provider);
    if (error) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
        e.preventDefault();
        // Signing in: Enter on the password step submits
        if (isLogin && authStep === 'password') formRef.current?.requestSubmit();
...
useEffect(() => {
    if (modalStatus === 'success' && !isLogin) {
        fireSideCanons();
    }
}, [modalStatus, isLogin]);

  // Rendered inline (not as <Modal />): a component defined inside another remounts every render,
  // which would restart the loading animation
  const renderModal = () => (
    <AnimatePresence>
        {modalStatus !== 'closed' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card/80 border-4 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2">
                    {(modalStatus === 'error' || modalStatus === 'success' || modalStatus === 'info') && <button onClick={closeModal} className="absolute top-2 right-2 p-1 text-muted-foreground hover:text-foreground transition-colors"><X className="w-5 h-5" /></button>}
                    {modalStatus === 'error' && <>
                    ...
                    {modalStatus === 'info' && <>
                        ...
                        <p className="text-lg font-medium text-foreground text-center">{infoMessage}</p>
                        <GlassButton onClick={closeModal} size="sm" className="mt-4">OK</GlassButton>
                    </>}
                    {modalStatus === 'loading' &&
                        <TextLoop interval={TEXT_LOOP_INTERVAL} stopOnEnd={true}>
                            {(isLogin ? loginSteps : modalSteps.slice(0, -1)).map((step, i) => 
                    ...
                    {modalStatus === 'success' &&
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <p className="text-lg font-medium text-foreground text-center">{successMessage}</p>
                        </div>
                    }
                </motion.div>
            </motion.div>
  ...
  return (
    <div className="bg-background min-h-screen w-screen flex flex-col">
        ...
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 sm:top-6">
            {logo}
            {brandName && <h1 className="text-base font-bold text-foreground">{brandName}</h1>}
        </div>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="flex justify-center text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight text-foreground whitespace-nowrap">{title}</p></div></BlurFade>
                        ...
                        <BlurFade delay={0.25 * 3}><div className="flex items-center justify-center gap-4 w-full">
                            <GlassButton type="button" onClick={() => handleSocial("google")} contentClassName="flex items-center justify-center gap-2" size="sm"><GoogleIcon /><span className="font-semibold text-foreground">Google</span></GlassButton>
                            <GlassButton type="button" onClick={() => handleSocial("github")} contentClassName="flex items-center justify-center gap-2" size="sm"><GitHubIcon /><span className="font-semibold text-foreground">GitHub</span></GlassButton>
                        </div></BlurFade>
                    ...
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="flex justify-center text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">{isLogin ? "Enter your password" : "Create your password"}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">{isLogin ? <>Signing in as<span className="block font-semibold text-foreground [overflow-wrap:anywhere]">{email}</span></> : "Your password must be at least 6 characters long."}</p></BlurFade>
                    </motion.div>}
                     {authStep === "confirmPassword" && <motion.div key="confirm-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                         <BlurFade delay={0} className="w-full"><div className="flex justify-center text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">One Last Step</p></div></BlurFade>
                                ...
                                {authStep === "password" && <BlurFade key="password-field" className="w-full">
                                    <div className="relative w-full">
                                        ...
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            ...
                                            <input ref={passwordInputRef} type={showPassword ? "text" : "password"} autoComplete={isLogin ? "current-password" : "new-password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none" />
                                            <div className={cn( "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out", isPasswordValid ? "w-10 pr-1" : "w-0" )}>{isLogin
                                                ? <GlassButton type="submit" size="icon" aria-label="Log in" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton>
                                                : <GlassButton type="button" onClick={handleProgressStep} size="icon" aria-label="Submit password" contentClassName="text-foreground/80 hover:text-foreground"><ArrowRight className="w-5 h-5" /></GlassButton>}</div>
                                        </div></div>
                                    </div>
                                    <BlurFade inView delay={0.2}><div className="mt-4 flex items-center justify-between">
                                        <button type="button" onClick={handleGoBack} className="flex items-center gap-2 text-sm text-foreground/70 hover:text-foreground transition-colors"><ArrowLeft className="w-4 h-4" /> Go back</button>
                                        {isLogin && onForgotPassword && <button type="button" onClick={handleForgot} className="text-sm font-semibold text-[#4f46e5] hover:underline">Forgot password?</button>}
                                    </div></BlurFade>
                                </BlurFade>}
                            </AnimatePresence>
                        </motion.div>}
                        ...
                        {authStep === 'confirmPassword' && <BlurFade key="confirm-password-field" className="w-full">
                            <div className="relative w-full">
                                ...
                                <div className="glass-input-wrap w-[300px]"><div className="glass-input">
                                    ...
                                    <input ref={confirmPasswordInputRef} type={showConfirmPassword ? "text" : "password"} autoComplete="new-password" placeholder="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none" />
                ...
                {footer && <div className="w-[300px] text-center">{footer}</div>}
            </fieldset>
        </div>
    </div>
  );
